#!/usr/bin/env python3
"""
Plot average capabilities vs computation time and makespan from TestRunManager CSV exports
Reads from average_capabilities/data/
Shows scaling across 6 automata and 3 robot configurations (6, 10, 16) with varying average capabilities
"""
...
for csv_file in sorted(csv_files):
    ...
    # Sort by average_capabilities
    rows_list.sort(key=lambda x: float(x['average_capabilities']))
    
    for row in rows_list:
        avg_cap = float(row['average_capabilities'])
        ...
        config_data[config_key]['avg_caps'].append(avg_cap)
...
# ============================================================================
# FIGURE 1: Computation Time vs Average Capabilities (All Configs Combined)
# ============================================================================
fig1, ax1 = plt.subplots(figsize=(12, 7))
fig1.suptitle('Average Capabilities vs Computation Time (All Configurations)', fontsize=16, fontweight='bold')
...
for (automaton_id, robot_count) in sorted(config_data.keys()):
    ...
    ax1.plot(data['avg_caps'], data['computation_times'], 
             marker=robot_markers[robot_count], markersize=8, 
             linewidth=2.5, label=label)

ax1.set_xlabel('Average Capabilities per Robot', fontsize=12, fontweight='bold')
ax1.set_ylabel('Computation Time (ms)', fontsize=12, fontweight='bold')
ax1.grid(True, alpha=0.3)
ax1.legend(loc='best', fontsize=9)

plt.tight_layout()
plt.savefig('Plots/average_capabilities_computation_time_all.png', dpi=300, bbox_inches='tight')
print("✓ Plot saved as Plots/average_capabilities_computation_time_all.png")
plt.close(fig1)

# ============================================================================
# FIGURE 2: Makespan vs Average Capabilities (All Configs Combined)
# ============================================================================
fig2, ax2 = plt.subplots(figsize=(12, 7))
fig2.suptitle('Average Capabilities vs Makespan (All Configurations)', fontsize=16, fontweight='bold')

for (automaton_id, robot_count) in sorted(config_data.keys()):
    data = config_data[(automaton_id, robot_count)]
    label = f'Auto {automaton_id}, {robot_count} Robots (Task Allocation)'
    ax2.plot(data['avg_caps'], data['makespans'], 
             marker=robot_markers[robot_count], markersize=8, 
             linewidth=2.5, label=label)
    ...
    if any(pm > 0 for pm in data['product_makespans']):
        ...
        ax2.plot(data['avg_caps'], data['product_makespans'], 
                marker=robot_markers[robot_count], markersize=6, 
                linewidth=2.5, linestyle='--', label=label_prod)
    
    # Add value labels on points
    for cap, m in zip(data['avg_caps'], data['makespans']):
        if m > 0:
            ax2.text(cap, m, f'{m:.0f}s', ha='center', va='bottom', fontsize=7)

ax2.set_xlabel('Average Capabilities per Robot', fontsize=12, fontweight='bold')
ax2.set_ylabel('Makespan (seconds)', fontsize=12, fontweight='bold')
ax2.grid(True, alpha=0.3)
ax2.legend(loc='best', fontsize=9)

plt.tight_layout()
plt.savefig('Plots/average_capabilities_makespan_all.png', dpi=300, bbox_inches='tight')
print("✓ Plot saved as Plots/average_capabilities_makespan_all.png")
plt.close(fig2)
...
for robot_count in [6, 10, 16]:
    ...
    fig.suptitle(f'Average Capabilities vs Computation Time ({robot_count}-Robot Team)', 
    ...
    for automaton_id in range(1, max_automaton_id + 1):
        ...
        if config_key in config_data:
            ...
            ax.plot(data['avg_caps'], data['computation_times'], 
                    marker='o', markersize=8, 
                    linewidth=2.5, color=automaton_colors[automaton_id-1],
                    label=f'Automaton {automaton_id}')
    
    ax.set_xlabel('Average Capabilities per Robot', fontsize=12, fontweight='bold')
    ax.set_ylabel('Computation Time (ms)', fontsize=12, fontweight='bold')
    ax.grid(True, alpha=0.3)
    ax.legend(loc='best', fontsize=10)
    
    plt.tight_layout()
    plt.savefig(f'Plots/average_capabilities_computation_time_{robot_count}robots.png', dpi=300, bbox_inches='tight')
    print(f"✓ Plot saved as Plots/average_capabilities_computation_time_{robot_count}robots.png")
    plt.close(fig)
...
for robot_count in [6, 10, 16]:
    fig, ax = plt.subplots(figsize=(12, 7))
    fig.suptitle(f'Average Capabilities vs Makespan ({robot_count}-Robot Team)', 
                 fontsize=16, fontweight='bold')
    ...
    for automaton_id in range(1, max_automaton_id + 1):
        ...
        if config_key in config_data:
            data = config_data[config_key]
            ax.plot(data['avg_caps'], data['makespans'], 
                    marker='o', markersize=8, 
                    linewidth=2.5, color=automaton_colors[automaton_id-1],
                    label=f'Automaton {automaton_id}')
    ...
    plt.tight_layout()
    plt.savefig(f'Plots/average_capabilities_makespan_{robot_count}robots.png', dpi=300, bbox_inches='tight')
    print(f"✓ Plot saved as Plots/average_capabilities_makespan_{robot_count}robots.png")
...
for (automaton_id, robot_count) in sorted(config_data.keys()):
    ...
    fig.suptitle(f'Automaton {automaton_id} ({robot_count} Robots): Average Capabilities vs Computation Time', 
    ...
    # Add value labels
    for cap, t in zip(data['avg_caps'], data['computation_times']):
        ax.text(cap, t, f'{t:.2f}ms', ha='center', va='bottom', fontsize=9)
    ...
    if any(m > 0 for m in data['makespans']):
        fig, ax = plt.subplots(figsize=(10, 6))
        fig.suptitle(f'Automaton {automaton_id} ({robot_count} Robots): Average Capabilities vs Makespan', 
                     fontsize=14, fontweight='bold')
        ...
        for cap, m in zip(data['avg_caps'], data['makespans']):
            if m > 0:
                ax.text(cap, m, f'{m:.0f}s', ha='center', va='bottom', fontsize=9)
